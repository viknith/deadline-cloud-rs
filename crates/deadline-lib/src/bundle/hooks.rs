...
use crate::bundle::submission::SubmissionHandler;

pub struct HookManager<'a> {
    pub(crate) job_bundle_dir: PathBuf,
    pub(crate) hooks: Option<HookConfiguration>,
...
impl<'a> HookManager<'a> {
    pub fn new(
        job_bundle_dir: &Path,
        handler: &'a dyn SubmissionHandler,
        hooks: Option<HookConfiguration>,
    ) -> Self {
    ...
    pub fn load_hooks(&mut self) -> Result<Option<&HookConfiguration>, DeadlineError> {
        let config_data =
            crate::bundle::loader::read_yaml_or_json_object(&self.job_bundle_dir, "hooks", false)?;
    ...
    pub fn execute_pre_submission_hooks(
        ...
        for (i, hook) in hooks.iter().enumerate() {
            ...
            self.handler.on_message(&format!(
...
fn report_failure(
    ...
    handler.on_message(&format!("Exit code: {}", result.exit_code));
    if result.timed_out {
        handler.on_message(&format!("Timed out after {}s", hook.timeout));
    }
    if !result.stdout.is_empty() {
        handler.on_message(&format!("stdout:\n{}", result.stdout));
    }
    if !result.stderr.is_empty() {
        handler.on_message(&format!("stderr:\n{}", result.stderr));
    }
}
...
mod tests {
    ...
    /// Test handler that discards all messages and auto-accepts.
    struct NullHandler;
    ...
    fn load_hooks_no_file() {
        ...
        let mut mgr = HookManager::new(dir.path(), &NullHandler, None);
    ...
    fn pre_hook_modifies_payload() {
        ...
        let dir_str = dir.path().to_str().unwrap();
        if cfg!(windows) {
            let script = dir.path().join("modify.cmd");
    ...
    fn pre_hook_timeout() {
        ...
        let dir_str = dir.path().to_str().unwrap();
        let (cmd, args) = if cfg!(windows) {
            ("cmd.exe", r#"["/c", "ping -n 10 127.0.0.1 >nul"]"#)
        } else {
            ("sh", r#"["-c", "sleep 10"]"#)
        };
        write_hooks_yaml(
    ...
    fn pre_hook_receives_stdin() {
        ...
        if cfg!(windows) {
            let script = dir.path().join("capture.cmd");
            std::fs::write(
                &script,
        ...
        assert_eq!(parsed["jobName"], "StdinTestJob");
    }
