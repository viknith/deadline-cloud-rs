...
fn bundle_settings() -> insta::Settings {
    ...
    // Windows extended-length path prefix (\\?\) appears in canonicalized paths
    settings.add_filter(r"\\\\\?\\", "");
    // Symlink escape test: normalize Windows target path to Unix equivalent
    settings.add_filter(r"C:\\Windows\\System32\\drivers\\etc\\hosts", "/etc/hosts");
    settings
}
