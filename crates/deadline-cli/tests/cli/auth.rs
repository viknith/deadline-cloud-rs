...
",
    ...
    .unwrap();
    let monitor_path = if cfg!(windows) {
        r"C:\nonexistent\path\to\monitor"
    } else {
        "/nonexistent/path/to/monitor"
    };
    std::fs::write(
        &harness.config_path,
        format!(
...
"
    ...
    settings.add_filter(
        r"(?:C:\\nonexistent\\path\\to\\monitor|/nonexistent/path/to/monitor)",
        "[MONITOR_PATH]",
    );
...
#[cfg(unix)] // timing-sensitive: .cmd timeout/append doesn't work reliably on Windows CI
#[tokio::test]
